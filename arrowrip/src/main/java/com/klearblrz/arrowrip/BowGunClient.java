 ...
 * Ammo is always the real vanilla arrow. Only the magazine counter/reload cycle is virtual.
 * The server remains authoritative for whether a bow shot is accepted, its damage and range.
 */
public final class BowGunClient implements ClientModInitializer {
    ...
    private static final int AUTO_DRAW_TICKS = 3;
    private static final int SEMI_DRAW_TICKS = 5;
    ...
    private static void tick(MinecraftClient client) {
        ...
        // Real-arrow automatic fire: start using the real bow, then release it after a short charge.
        if (client.player.isUsingItem() && client.player.getActiveItem().isOf(Items.BOW)) {
            ...
            int needed = autoMode ? AUTO_DRAW_TICKS : SEMI_DRAW_TICKS;
            boolean canRelease = autoMode || !semiLatched;

            if (used >= needed && !releasedThisUse && canRelease && client.interactionManager != null) {
                client.interactionManager.stopUsingItem(client.player);
        ...
        // Keep right click held: AUTO repeatedly starts the real vanilla bow again.
    ...
    private static void renderHud(DrawContext ctx) {
        ...
        String ammo = (autoMode ? "AUTO  " : "SEMI  ") + magazine + "/" + MAG_SIZE + "  OK";
    ...
    public static int muzzleFlashTicks() { return recoilTicks > 3 ? 2 : 0; }
}
