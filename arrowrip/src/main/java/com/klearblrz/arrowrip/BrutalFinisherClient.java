...
public final class BrutalFinisherClient implements ClientModInitializer {
    ...
    private static int selectedFinisherTrack = -1; // -1 = AUTO
    private static int selectedBackgroundTrack = 0;
    ...
    private static volatile Player finisherPlayer;
    private static volatile boolean backgroundPlaying;
    private static boolean resumeBackgroundAfterFinisher;
    ...
    public void onInitializeClient() {
        menuKey = KeyBindingHelper.registerKeyBinding(new KeyBinding(
                "key.arrowrip.finisher_menu", InputUtil.Type.KEYSYM, GLFW.GLFW_KEY_H, ArrowRipClient.CATEGORY));
    ...
    public static void cycleFinisherTrack() {
        selectedFinisherTrack++;
        if (selectedFinisherTrack >= TRACKS.length) selectedFinisherTrack = -1;
    }

    public static String getFinisherTrackName() {
        return selectedFinisherTrack < 0 ? "AUTO (ana göre)" : TRACKS[selectedFinisherTrack].name;
    }

    public static void cycleBackgroundTrack() {
        selectedBackgroundTrack = (selectedBackgroundTrack + 1) % TRACKS.length;
    }

    public static String getBackgroundTrackName() {
        return TRACKS[selectedBackgroundTrack].name;
    }
    ...
    public static void playBackgroundSelected() {
        ...
        stopBackgroundMusic();
        startBackgroundMusic(TRACKS[selectedBackgroundTrack].path);
    }

    public static void stopBackgroundMusic() {
        try {
            Player p = backgroundPlayer;
            if (p != null) p.close();
    ...
    public static void previewCurrentTarget(MinecraftClient client) {
        if (client == null || client.player == null || client.world == null || finisherActive) return;
    ...
    private static void tick(MinecraftClient client) {
        ...
        while (menuKey.wasPressed()) {
            if (!finisherActive && client.currentScreen == null) client.setScreen(new FinisherMenuScreen());
        }
    ...
    private static Track chooseAutoTrack() {
        return switch (selectedMode) {
            case SWORD_RAIN -> TRACKS[ThreadLocalRandom.current().nextInt(0, 3)];
            case DECAPITATION -> TRACKS[ThreadLocalRandom.current().nextInt(3, 5)];
            case MIXED -> TRACKS[ThreadLocalRandom.current().nextInt(TRACKS.length)];
        };
    }
    ...
    private static void startFinisher(MinecraftClient client, PlayerEntity self, Vec3d origin) {
        ...
        resumeBackgroundAfterFinisher = backgroundPlaying;
        if (backgroundPlaying) stopBackgroundMusic();
        ...
        finisherTick = 0;
        beatIndex = 0;
        activeTrack = selectedFinisherTrack < 0 ? chooseAutoTrack() : TRACKS[selectedFinisherTrack];
        ...
        startFinisherMusic(activeTrack.path);
    ...
    private static void finishFinisher(MinecraftClient client) {
        ...
        if (resumeBackgroundAfterFinisher) {
            resumeBackgroundAfterFinisher = false;
            startBackgroundMusic(TRACKS[selectedBackgroundTrack].path);
        }
    }
    ...
    private static void decapitationEffect(MinecraftClient client) {
        ...
        if (client.player != null) client.player.playSound(SoundEvents.ENTITY_PLAYER_HURT, 0.55f, 0.46f);
    ...
    private static void startBackgroundMusic(String resourcePath) {
        Thread t = new Thread(() -> {
        ...
        }, "ArrowRip-Background-Music");
        t.setDaemon(true);
        t.start();
    }

    private static void startFinisherMusic(String resourcePath) {
        Thread t = new Thread(() -> {
            try (InputStream in = BrutalFinisherClient.class.getResourceAsStream(resourcePath)) {
                if (in == null) return;
                Player p = new Player(in);
    ...
    private static void stopFinisherMusic() {
        try {
            Player p = finisherPlayer;
            if (p != null) p.close();
        } catch (Exception ignored) {}
        finisherPlayer = null;
    }
